...
from ...integrations.auth.credentials import CredentialNotFoundError
from ...integrations.auth.codex import CodexAuthError, get_codex_status
from ...integrations.auth.copilot import CopilotAuthError, get_copilot_status
...
from ...modules.llm.reasoning import normalize_reasoning_effort
...
def is_llm_configured(config: Config) -> bool:
    """Return whether the active provider has the runtime credentials it needs."""
    if not config.llm.providers or not config.llm.default or config.llm.default not in config.llm.providers:
        return False

    provider = config.llm.providers[config.llm.default]
    profile = resolve_runtime_provider_profile(provider, provider_name=config.llm.default)
    app_home = config.source_path.parent if config.source_path is not None else None

    if profile.auth_type == OPENAI_CODEX_OAUTH_AUTH_TYPE:
    ...
    if profile.auth_type == GITHUB_COPILOT_OAUTH_AUTH_TYPE:
        if not provider.model:
            return False
        try:
    ...
    if profile.auth_type == OPTIONAL_API_KEY_AUTH_TYPE:
...
class DefaultLlmRuntimeFactory:
    """Application-owned implementation of the agent's LLM runtime factory port."""
    ...
    def create_configured(
        ...
        *,
        fallback_app_home: str | Path | None = None,
    ) -> tuple[LLMProvider, ResolvedProviderRuntime]:
        return create_configured_llm(config, fallback_app_home=fallback_app_home)
