"""
opensprite/app/channels/runtime.py - 訊息頻道 runtime
...
from ...core.contracts.channel_identity import normalize_identifier as _normalize_identifier
from opensprite.core.logging import logger
from . import adapters as _channel_adapters


CHANNEL_FACTORIES = _channel_adapters.CHANNEL_ADAPTER_FACTORIES
...
class ChannelRuntimeManager:
    ...
    def _enabled_instances(self, channels_config: Any, *, include_fixed: bool) -> dict[str, dict[str, Any]]:
        ...
        for instance_id, channel_config in sections.items():
            if not isinstance(channel_config, dict) or not channel_config.get("enabled"):
                continue
            normalized_id = _normalize_identifier(instance_id, fallback=instance_id)
            if not include_fixed and normalized_id in FIXED_RUNTIME_INSTANCES:
                continue
            channel_type = _normalize_identifier(str(channel_config.get("type") or normalized_id), fallback="")
    ...
    async def start_instance(self, instance_id: str, channel_config: dict[str, Any]) -> bool:
        ...
        channel_type = _normalize_identifier(str(channel_config.get("type") or instance_id), fallback="")
...
__all__ = ["CHANNEL_FACTORIES", "ChannelRuntimeManager", "start_channels"]
