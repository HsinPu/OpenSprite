...
def auth_logout_command(*, provider: str, config: str | None, resolve_app_home: Callable[[str | None], Path]) -> None:
    ...
    app_home = resolve_app_home(config)
    if provider == "copilot":
        from ...integrations.auth.copilot import copilot_auth_path, delete_copilot_token
    ...
    from ...integrations.auth.codex import codex_auth_path, delete_codex_token
...
def auth_login_command(*, provider: str, config: str | None, timeout_seconds: float, resolve_app_home: Callable[[str | None], Path]) -> None:
    ...
    if provider == "copilot":
        import time
        from ...integrations.auth.copilot import (
            CopilotAuthError,
            copilot_auth_path,
    ...
    from ...integrations.auth.codex import CodexAuthError, codex_auth_path, codex_device_login
...
def auth_credentials_list_command(
    *,
    ...
    from ...integrations.auth.credentials import CredentialStoreError, list_credentials
