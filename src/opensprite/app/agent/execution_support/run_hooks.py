"""Agent execution hooks and trace metadata helpers."""
...
from ....core.contracts.bus_events import OutboundMessage
...
from ....integrations.mcp.naming import (
    is_mcp_tool_name as _is_mcp_tool_name,
    mcp_tool_display_name as _mcp_tool_display_name,
)
...
def tool_warrants_progress_notice(tool_name: str | None) -> bool:
    return (
        str(tool_name or "").strip() in PROGRESS_NOTICE_TOOL_NAMES
        or _is_mcp_tool_name(tool_name)
    )
...
class RunHookService:
    ...
    def format_tool_progress_message(tool_name: str, tool_args: dict[str, Any]) -> str:
        ...
        if _is_mcp_tool_name(tool_name):
            tail = _mcp_tool_display_name(tool_name)
