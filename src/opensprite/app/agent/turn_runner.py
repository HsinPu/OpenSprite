...
from ...modules.media.audio_input import AudioInputPreprocessor
from ...modules.media.turn_policy import (
...
from ...core.run_tracking.state import AgentRunStateService, RunBusyError
from ...core.contracts.run_events import AUDIO_INPUT_TRANSCRIBED_EVENT, EXECUTION_STOPPED_EVENT, LLM_STATUS_EVENT
from ...core.contracts.run_lifecycle import RUN_COMPLETED_STATUS, RUN_STOPPED_STATUS
...
from ...modules.runs.turn_lifecycle import RunLifecycleService
...
class AgentTurnRunner:
    ...
    def __init__(
        ...
        *,
        ...
        response_finalizer: RunResponseFinalizer,
        ...
        consume_delegated_task_updates: Callable[
            [str],
            tuple[delegation_contracts.StoredDelegatedTask, ...],
        ],
    ...
    def _user_history_content(user_message: UserMessage, turn: PreparedTurnInput) -> str:
        ...
        if turn.image_files or turn.audio_files or turn.video_files:
            if failed_events:
                return format_partially_saved_media_history_content(
                    image_files=turn.image_files,
                    audio_files=turn.audio_files,
                    video_files=turn.video_files,
                    failed_count=len(failed_events),
                )
            return format_saved_media_history_content(
                image_files=turn.image_files,
                audio_files=turn.audio_files,
                video_files=turn.video_files,
            )
        return format_failed_media_history_content()
