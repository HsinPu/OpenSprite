"""Delegated subagent run orchestration and tool access policy."""
...
from ...core.contracts.run_lifecycle import RUN_STARTED_EVENT
from ...core.run_tracking.state import RunCancelledError
from ...modules.runs.trace_recorder import RunTraceRecorder
from ...modules.subagents.model_routing import ModelRoutedProvider
from ...modules.subagents.profiles import (
...
from . import delegation_contracts
from ...integrations.subagents.prompts import get_all_subagents, load_metadata
from ...core.contracts.tool_names import (
    DELEGATE_MANY_TOOL_NAME,
    DELEGATE_TOOL_NAME,
)
from ...modules.tools.registry import ToolRegistry
from ...core.contracts.tool_results import classify_tool_result_status, tool_error_result
from opensprite.core.logging import logger
from .tool_selection import ToolSelectionResolver
...
def build_subagent_tool_registry(
    ...
    *,
    ...
    profile = profile_for_subagent(
        prompt_type,
        tool_profile=metadata.get(TOOL_PROFILE_METADATA_FIELD),
    )
...
class SubagentRunService:
    ...
    def __init__(
        ...
        *,
        ...
        llm_runtime_factory: LlmRuntimeFactory | None,
        ...
        record_delegated_task_update: Callable[
            [str | None, delegation_contracts.StoredDelegatedTask],
            None,
        ],
    ):
        ...
        *,
        ...
        if llm_provider:
            ...
            if provider_config is not None and getattr(provider_config, "enabled", True):
                ...
                provider_override, _ = self._llm_runtime_factory.create_provider(
                    provider_config,
    ...
    def _record_task_update(
        ...
        *,
        ...
        self._record_delegated_task_update(
            ...
            delegation_contracts.StoredDelegatedTask(
