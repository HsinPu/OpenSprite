...
from ...core.contracts.messages import AssistantMessage, UserMessage
from ...core.contracts.prompt_history import PreparedPromptHistory
from ...core.contracts.llm import ChatMessage
from ...core.ports.llm import LLMProvider
from ...core.ports.llm_runtime import LlmRuntimeFactory
from ...core.ports.context import ContextBuilder
from ...core.ports.storage import StorageProvider
from ...integrations.workspace.paths import (
    get_session_curator_state_file,
    get_session_learning_state_file,
    get_session_skills_dir,
    get_session_workspace,
)
from ...integrations.documents.recent_summary import RecentSummaryStore as _RecentSummaryStore
from ...integrations.documents.memory import MemoryStore as _MemoryStore
from ...integrations.documents.user_profile import create_user_profile_store as _create_user_profile_store
from ...integrations.documents.curator_state import CuratorStateStore as _CuratorStateStore
from ...integrations.documents.learning import JsonLearningLedgerStore as _JsonLearningLedgerStore
from ...integrations.documents.user_overlay import (
    UserOverlayIndexStore as _UserOverlayIndexStore,
    UserOverlayStore as _UserOverlayStore,
)
from ...integrations.persistence.media import InboundMediaPersistence
from ...modules.documents.recent_summary import (
    RecentSummaryConsolidator as _RecentSummaryConsolidator,
    RecentSummaryUpdateService as _RecentSummaryUpdateService,
)
from ...modules.documents.user_profile import (
    UserProfileConsolidator as _UserProfileConsolidator,
    UserProfileUpdateService as _UserProfileUpdateService,
)
from ...modules.media.router import MediaRouter
from ...modules.documents.user_overlay import UserOverlayPromotionService as _UserOverlayPromotionService
from ...core.ports.search import SearchStore
from ...modules.tools.registry import ToolRegistry
from ...core.contracts.tool_results import classify_tool_result_status
from ...integrations.processes.background_runtime import (
    BackgroundProcessManager,
    BackgroundSession,
)
from ...modules.documents.identity import resolve_user_overlay_id
from ...core.run_tracking.state import AgentRunStateService
from ...core.contracts.tool_names import (
    PROCESS_TOOL_NAME,
)
...
from ...modules.documents.curator import CuratorService as _CuratorService
from ...modules.documents.memory_consolidation import MemoryConsolidationService as _MemoryConsolidationService
from ...modules.documents.learning import LearningLedger as _LearningLedger
from ...modules.conversations.history import MessageHistoryService as _MessageHistoryService
from ...modules.conversations.reset import HistoryResetService as _HistoryResetService
from ...modules.documents.skill_review import SkillReviewService as _SkillReviewService
from ...integrations.documents.fingerprints import fingerprint_text_directory as _fingerprint_text_directory
...
from ...modules.runs.file_changes import RunFileChangeService
from ...modules.runs.response_finalizer import RunResponseFinalizer
from ...modules.runs.trace_recorder import RunTraceRecorder
from ...integrations.mcp.lifecycle import McpLifecycleService
from ...integrations.processes.background_session_notifications import (
    BackgroundSessionNotificationService,
)
from .execution_support.run_hooks import RunHookService
...
class AgentLoop:
    ...
    def __init__(
        ...
        *,
        ...
        media_router_reloader: MediaRouterReloader | None = None,
        llm_runtime_factory: LlmRuntimeFactory | None = None,
        default_tool_registrar: DefaultToolRegistrar | None = None,
        memory_tool_registrar: MemoryToolRegistrar | None = None,
        web_search_tool_reloader: WebSearchToolReloader | None = None,
        browser_tool_reloader: BrowserToolReloader | None = None,
    ):
        ...
        self.config = config
        ...
        self._media_router_reloader = media_router_reloader
        self._llm_runtime_factory = llm_runtime_factory
        self._memory_tool_registrar = memory_tool_registrar
        self._web_search_tool_reloader = web_search_tool_reloader
        self._browser_tool_reloader = browser_tool_reloader
        ...
        self.curator: _CuratorService | None = None
        self.learning_ledger: _LearningLedger | None = None
        ...
        self.message_history = _MessageHistoryService(
        ...
        self.history_reset = _HistoryResetService(
            storage=self.storage,
            history_search_store=self.history_search_store,
        ...
        self.user_overlay_promotion = _UserOverlayPromotionService(
        ...
        self.media_service = InboundMediaPersistence(
            workspace_for_session=self._get_workspace_for_session,
        )
        ...
        self.tools = setup_agent_tools(
            self,
            tools,
            default_tool_registrar=default_tool_registrar,
        )
        self.subagents = SubagentRunService(
            ...
            llm_runtime_factory=self._llm_runtime_factory,
        ...
        self.skill_review = _SkillReviewService(
            storage=self.storage,
            tools=self.tools,
    ...
    def _maybe_update_user_overlay(self, session_id: str) -> dict[str, Any] | None:
        ...
        profile_store = _create_user_profile_store(
            self.app_home,
            session_id,
    ...
    def _setup_context_builder(self, context_builder: ContextBuilder | None) -> ContextBuilder:
        ...
        if context_builder is None:
            try:
                from ...integrations.workspace.paths import get_app_home, get_tool_workspace
                from ...integrations.context.file_builder import FileContextBuilder
                from ...integrations.workspace.bootstrap import sync_templates
    ...
    def _setup_learning_ledger(self) -> _LearningLedger:
        """Create the session learning ledger and attach it to compatible context builders."""
        ledger = _LearningLedger(
            store=_JsonLearningLedgerStore(
    ...
    def _setup_memory_store(self) -> _MemoryStore:
        """Create the long-term memory store."""
        memory_dir = getattr(self._context_builder, "memory_dir", Path.cwd() / "memory")
        return _MemoryStore(memory_dir, app_home=self.app_home, workspace_root=self.tool_workspace)

    def _setup_memory_consolidation(self) -> _MemoryConsolidationService:
        """Create the memory consolidation maintenance service."""
        return _MemoryConsolidationService(
            storage=self.storage,
    ...
    def _setup_user_profile_update(self) -> _UserProfileUpdateService:
        """Create the optional USER.md update service."""
        consolidator: _UserProfileConsolidator | None = None
        if self.app_home is not None:
            bootstrap_dir = getattr(self._context_builder, "bootstrap_dir", None)
            consolidator = _UserProfileConsolidator(
                storage=self.storage,
                provider=self.provider,
                model=self.provider.get_default_model(),
                profile_store_factory=lambda session_id: _create_user_profile_store(
                    self.app_home,
                    session_id,
                    bootstrap_dir=bootstrap_dir,
                    workspace_root=self.tool_workspace,
        ...
        return _UserProfileUpdateService(consolidator)

    def _setup_recent_summary_update(self) -> _RecentSummaryUpdateService:
        """Create the optional RECENT_SUMMARY.md update service."""
        memory_dir = getattr(self._context_builder, "memory_dir", None)
        if memory_dir is None:
            return _RecentSummaryUpdateService(None)

        summary_store = _RecentSummaryStore(memory_dir, app_home=self.app_home, workspace_root=self.tool_workspace)
        consolidator = _RecentSummaryConsolidator(
            storage=self.storage,
            provider=self.provider,
            model=self.provider.get_default_model(),
        ...
        return _RecentSummaryUpdateService(consolidator)

    def _setup_curator(self) -> _CuratorService:
        ...
        return _CuratorService(
            ...
            state_store=_CuratorStateStore(
    ...
    def _clear_recent_summary(self, session_id: str) -> None:
        ...
        if memory_dir is None:
            return
        _RecentSummaryStore(memory_dir, app_home=self.app_home, workspace_root=self.tool_workspace).clear(session_id)
    ...
    def _register_memory_tool(self) -> None:
        ...
        if self._memory_tool_registrar is None:
            return
        self._memory_tool_registrar(
            self.tools,
            self.memory,
    ...
    def reload_llm_from_config(self, config: Config) -> dict[str, Any]:
        ...
        if self._llm_runtime_factory is None:
            raise RuntimeError("LLM runtime factory is not configured")
        return reload_agent_llm_from_config(
            self,
            config,
            llm_runtime_factory=self._llm_runtime_factory,
        )
    ...
    def reload_media_from_config(self, config: Config) -> dict[str, Any]:
        ...
        if self._media_router_reloader is None:
            raise RuntimeError("Media runtime reload is unavailable without application composition.")
        return reload_agent_media_from_config(
            self,
            config,
            media_router_reloader=self._media_router_reloader,
        )
    ...
    def reload_web_search_from_config(self, config: Config) -> dict[str, Any]:
        ...
        if self._web_search_tool_reloader is None:
            raise RuntimeError(
                "Web search tool reload is unavailable without application composition."
            )
        return reload_agent_web_search_from_config(
            self,
            config,
            tool_reloader=self._web_search_tool_reloader,
        )
    ...
    def reload_browser_from_config(self, config: Config) -> dict[str, Any]:
        ...
        if self._browser_tool_reloader is None:
            raise RuntimeError(
                "Browser tool reload is unavailable without application composition."
            )
        return reload_agent_browser_from_config(
            self,
            config,
            tool_reloader=self._browser_tool_reloader,
        )
    ...
    def _read_recent_summary_snapshot(self, session_id: str) -> str:
        ...
        if memory_dir is None:
            return ""
        return _RecentSummaryStore(memory_dir, app_home=self.app_home, workspace_root=self.tool_workspace).read(session_id)
    ...
    def _read_user_profile_snapshot(self, session_id: str) -> str:
        ...
        if self.app_home is None:
            return ""
        bootstrap_dir = getattr(self._context_builder, "bootstrap_dir", None)
        store = _create_user_profile_store(
            self.app_home,
            session_id,
            bootstrap_dir=bootstrap_dir,
            workspace_root=self.tool_workspace,
    ...
    def _read_skill_snapshot(self, session_id: str) -> str:
        ...
        if workspace is None:
            return ""
        return _fingerprint_text_directory(get_session_skills_dir(session_id, workspace_root=workspace))
    ...
    async def reset_history(self, session_id: str | None = None) -> None:
        ...
        """
        session_ids = [session_id] if session_id else await self.storage.get_all_sessions()
        for current_session_id in session_ids:
            if self.cron_manager is None:
                await self.history_reset.reset(current_session_id)
                continue
            async with self.cron_manager.quiesce_session(current_session_id):
                await self.history_reset.reset(current_session_id)
