...
from opensprite.integrations.processes import ripgrep as ripgrep_runtime
from opensprite.integrations.workspace.paths import build_workspace_resolver, resolve_workspace_path
...
from opensprite.modules.workspace.instructions import sanitize_workspace_instruction as _sanitize_workspace_instruction
...
_AGENT_HINT_FULL_READ_GUIDANCE = "Use read_file to read the full file."
...
def _config_write_blocked_paths(
    config_path_resolver: ConfigPathResolver | None,
) -> frozenset[Path] | None:
    """Load runtime-configured paths that filesystem tools must not write."""
    if config_path_resolver is None:
        return None
    ...
    try:
        ...
        return Config.tool_write_blocked_paths(cfg)
    except Exception:
        return None
...
def _write_guard(
    ...
    *,
    ...
    protection = evaluate_workspace_write_protection(
        file_path,
        blocked_paths_resolver=lambda: _config_write_blocked_paths(config_path_resolver),
    )
    if protection:
        return guard_result(protection.message, protection.category)
...
def _workspace_agents_hint(workspace: Path, target_path: Path, seen_paths: set[Path]) -> str:
    ...
    for directory in [start_dir, *start_dir.parents]:
        ...
        body = _sanitize_workspace_instruction(
            content,
            "AGENTS.md",
            max_chars=_MAX_AGENT_HINT_CHARS,
            full_read_guidance=_AGENT_HINT_FULL_READ_GUIDANCE,
        )
...
async def _ripgrep_files(workspace: Path, search_path: Path, pattern: str) -> list[Path] | None:
    ...
    rg = ripgrep_runtime.find_ripgrep()
    if rg is None:
        return None

    search_arg = _display_path(workspace, search_path) or "."
    returncode, stdout, _stderr = await ripgrep_runtime.run_ripgrep(
    ...
    for raw_line in stdout.splitlines():
        ...
        if not relative_path:
            continue
        file_path = resolve_workspace_path(workspace, relative_path)
...
async def _ripgrep_search(
    ...
    returncode, stdout, stderr = await ripgrep_runtime.run_ripgrep(
        args,
        workspace,
        timeout_seconds=_SEARCH_TIMEOUT_SECONDS,
    )
    ...
    for raw_line in stdout.splitlines():
        ...
        file_path = resolve_workspace_path(workspace, path_part)
        if file_path is None or not file_path.is_file():
            continue
...
class GlobFilesTool(Tool):
    ...
    async def _execute(self, **kwargs: Any) -> str:
        try:
            ...
            workspace = self._get_workspace()
            search_path = resolve_workspace_path(workspace, path)
...
class ApplyPatchTool(Tool):
    ...
    async def _execute(self, **kwargs: Any) -> str:
        try:
            ...
            diagnostics_text, diagnostics_failed = format_post_edit_diagnostics(diagnostic_inputs)
...
class WriteFileTool(Tool):
    ...
    async def _execute(self, **kwargs: Any) -> str:
        try:
            ...
            diagnostics_text, diagnostics_failed = format_post_edit_diagnostics([(file_path, display_path, content)])
...
class EditFileTool(Tool):
    ...
    async def _execute(self, **kwargs: Any) -> str:
        try:
            ...
            diagnostics_text, diagnostics_failed = format_post_edit_diagnostics([(file_path, display_path, new_content)])
