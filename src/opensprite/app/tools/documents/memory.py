...
from opensprite.modules.documents.safety import DurableMemorySafetyError as _DurableMemorySafetyError
...
class SaveMemoryTool(Tool):
    ...
    def __init__(self, memory_store: _MemoryStore, get_session_id: Callable[[], str | None]):
    ...
    async def _execute(self, memory_update: str, **kwargs: Any) -> str:
        ...
        if memory_update != current:
            ...
            except _DurableMemorySafetyError as exc:
                return _save_memory_error_result(
                    str(exc),
