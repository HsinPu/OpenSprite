...
from . import catalog as _channel_catalog
...
def _has_required_secret(channel_type: str, channel: dict[str, Any]) -> bool:
    spec = _channel_catalog.CHANNEL_TYPES.get(channel_type)
...
def _serialize_instance(
    ...
    is_registered_channel_type: Callable[[str], bool],
) -> dict[str, Any]:
    ...
    return {
        ...
        "registered": is_registered_channel_type(channel_type),
...
def _serialize_type(channel_type: str) -> dict[str, Any]:
    spec = _channel_catalog.CHANNEL_TYPES[channel_type]
    return {
        "id": spec.type_id,
        "type": spec.type_id,
        "name": spec.name,
...
class ChannelSettingsService:
    ...
    def list_channels(self) -> dict[str, Any]:
        ...
        available = [
            _serialize_type(spec.type_id)
            for spec in _channel_catalog.list_connectable_channel_types()
        ]
        return {
            "connected": connected,
            "available": available,
    ...
    def create_channel(self, channel_type: str, *, name: str | None, token: str | None) -> dict[str, Any]:
        ...
        if normalized_type not in _channel_catalog.CHANNEL_TYPES or normalized_type not in {
            spec.type_id for spec in _channel_catalog.list_connectable_channel_types()
        }:
            raise ChannelSettingsNotFound(f"Unknown connectable channel type: {channel_type}")

        normalized_name = (
            _coerce_text(name, field="name")
            or _channel_catalog.CHANNEL_TYPES[normalized_type].name
        )
        normalized_token = _coerce_text(token, field="token")
        if _channel_catalog.CHANNEL_TYPES[normalized_type].requires_token and not normalized_token:
            raise ChannelSettingsValidationError("token is required when connecting a new channel")

        main_data, instances = self._load_state()
        instance_id = _channel_catalog.make_unique_instance_id(
            instances,
            normalized_type,
            normalized_name,
        )
        channel = _channel_catalog.default_instance_config(
            normalized_type,
