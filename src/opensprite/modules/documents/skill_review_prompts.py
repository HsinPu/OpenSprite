...
from ...core.contracts.tool_names import (
    CONFIGURE_SKILL_TOOL_NAME as _CONFIGURE_SKILL_TOOL_NAME,
    READ_SKILL_TOOL_NAME as _READ_SKILL_TOOL_NAME,
)
...
You may ONLY use these tools: `{_READ_SKILL_TOOL_NAME}`, `{_CONFIGURE_SKILL_TOOL_NAME}`.
...
{_curator_shared_rules("session skills")}

Rules:
- Prefer `action=upsert` on an existing skill when refining; use `action=add` only for a genuinely new skill id. Use `{_READ_SKILL_TOOL_NAME}` with `skill-creator-design` before authoring a new skill.
...
- Use `{_CONFIGURE_SKILL_TOOL_NAME}` for the session workspace `skills/` folder only. Bundled skills live read-only under `~/.opensprite/skills/<id>/`.
"""
