...
from ...core.contracts.run_events import (
...
from .scheduler import CoalescingTaskScheduler as _CoalescingTaskScheduler
from ...core.contracts.documents import CuratorTurnResult
from .jobs import (
    CuratorJob as _CuratorJob,
    CuratorMaintenanceServices as _CuratorMaintenanceServices,
    CuratorRequest as _CuratorRequest,
    SessionRunner as _SessionRunner,
    SnapshotReader as _SnapshotReader,
)
from .scope import (
    CURATOR_MAINTENANCE_JOB_KEYS as _CURATOR_MAINTENANCE_JOB_KEYS,
    _ordered_maintenance_job_keys as _order_curator_maintenance_job_keys,
...
class _CuratorStateStore(Protocol):
    """Persistence boundary required by the curator orchestrator."""
...
class CuratorService:
    ...
    def __init__(
        self,
        *,
        maybe_consolidate_memory: _SessionRunner,
        maybe_update_recent_summary: _SessionRunner,
        maybe_update_user_profile: _SessionRunner,
        run_skill_review: _SessionRunner,
        should_run_skill_review: SkillReviewDecider,
        read_memory_snapshot: _SnapshotReader,
        read_recent_summary_snapshot: _SnapshotReader,
        read_user_profile_snapshot: _SnapshotReader,
        read_skill_snapshot: _SnapshotReader,
        emit_run_event: RunEventEmitter,
        state_store: _CuratorStateStore,
        record_learning: LearningRecorder | None = None,
    ):
        self._maintenance_services = _CuratorMaintenanceServices(
        ...
        self._requests: dict[str, _CuratorRequest] = {}
        self._active_requests: dict[str, _CuratorRequest] = {}
        self._runtime_state: dict[str, dict[str, Any]] = {}
        self._scheduler = _CoalescingTaskScheduler[str](
        ...
        self._skill_job = _CuratorJob("skills", "skills", read_skill_snapshot, self._skill_review_runner)
    ...
    def _record_run(
        ...
        *,
        ...
        state["run_count"] = _safe_int(state.get("run_count")) + 1
    ...
    @staticmethod
    def _merge_request(current: _CuratorRequest | None, incoming: _CuratorRequest) -> _CuratorRequest:
        if current is None:
            return incoming
        return _CuratorRequest(
            ...
            maintenance_job_keys=_order_curator_maintenance_job_keys([
    ...
    def status(self, session_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "run_count": _safe_int(session_state.get("run_count")),
    ...
    def _schedule(self, request: _CuratorRequest) -> bool:
    ...
    async def _emit_event(self, request: _CuratorRequest, event_type: str, payload: dict[str, Any]) -> None:
    ...
    async def _run_snapshot_job(self, session_id: str, job: _CuratorJob) -> tuple[bool, Any]:
    ...
    async def _run_request(self, session_id: str) -> None:
        ...
        try:
            ...
            selected_jobs: list[_CuratorJob] = []
