...
from ....core.contracts.search import SearchHit
from ....core.ports.search import SearchStore
from ....modules.search import query_policy
from ....modules.search.indexing import SearchChunkPayload, build_history_chunks
...
def _unicode_token_match(value: str | None, token: str | None) -> int:
    if value is None or token is None:
        return 0
    return int(query_policy.find_history_search_token_offset(value, token) is not None)
...
class SQLiteSearchStore(SearchStore):
    ...
    def __init__(
        ...
        *,
        ...
        self.history_top_k = query_policy.bound_history_search_limit(
            history_top_k,
            default=5,
        )
    ...
    def _get_conn(self) -> sqlite3.Connection:
        ...
        conn = sqlite_database.open_sqlite_connection(self.path)
        conn.create_function(
            "unicode_casefold",
            1,
            query_policy.unicode_casefold,
            deterministic=True,
        )
        conn.create_function(
            "unicode_token_match",
    ...
    async def index_message(
        ...
        async with self._lock:
            conn = self._get_conn()
            try:
                message_id = sqlite_database.find_message_id(
                    conn,
    ...
    async def search_history(
        ...
        if not query:
            return []
        requested_limit = query_policy.bound_history_search_limit(
            limit,
            default=self.history_top_k,
        )
    ...
    def _query_tokens(query: str) -> list[str]:
        normalized = query_policy.validate_history_search_query(query)
        _, tokens = query_policy.parse_history_search_terms(normalized)
        return tokens
    ...
    def _search_fts(
        ...
        *,
        ...
        if match_query is None:
            return []
        literals, _ = query_policy.parse_history_search_terms(query)
        normalized_literals = [
            query_policy.unicode_casefold(identifier)
            for identifier in literals
        ]
    ...
    def _search_substring(
        ...
        *,
        ...
        normalized_tokens = [
            query_policy.unicode_casefold(token)
            for token in tokens
        ]
