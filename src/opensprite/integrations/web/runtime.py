...
from ..processes.subprocess_control import windows_hidden_process_kwargs
...
def _default_frontend_source_dir(module_path: Path) -> Path:
    """Locate the source-checkout frontend without depending on module depth."""
    resolved_module = module_path.expanduser().resolve(strict=False)
    for parent in resolved_module.parents:
        if parent.name == "opensprite" and (parent / "__init__.py").is_file():
            return parent.parent.parent / "frontend"

    parents = resolved_module.parents
    if len(parents) > 3:
        return parents[3] / "frontend"
    return resolved_module.parent / "frontend"
...
def resolve_frontend_source_dir(config: Mapping[str, Any], *, module_path: Path) -> Path | None:
    ...
    if not configured_static:
        candidates.extend(
            [
                _default_frontend_source_dir(module_path),
                Path.cwd() / "frontend",
            ]
        )
...
def resolve_frontend_dir(config: Mapping[str, Any], *, module_path: Path) -> Path | None:
    ...
    candidates.extend(
        [
            _default_frontend_source_dir(module_path) / "dist",
