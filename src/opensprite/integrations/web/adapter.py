"""
opensprite/integrations/web/adapter.py - WebSocket chat adapter
...
from ...core.contracts.channel_identity import build_session_id, channel_from_session, normalize_identifier
from ...core.contracts.bus_events import RunEvent, SessionStatusEvent
from ...core.contracts.messages import AssistantMessage, UserMessage
from ...core.ports.channels import MessageAdapter
...
import opensprite.modules.channels.catalog as _channel_catalog
from ...modules.runs.presentation import serialize_run_event
from ...core.serialization import json_safe_payload
from opensprite.core.logging import logger
from . import api as web_api
from .settings import support as web_settings_support
...
class WebAdapter(MessageAdapter):
    ...
    def __init__(
        ...
        *,
        is_registered_channel_type: Callable[[str], bool] | None = None,
    ):
        self.mq = mq
        ...
        self._is_registered_channel_type = (
            is_registered_channel_type
            if is_registered_channel_type is not None
            else _channel_catalog.CHANNEL_TYPES.__contains__
        )
        ...
        self._api = web_api.WebApiHandlers(self)
    ...
    def _run_frontend_command(self, source_dir: Path, args: list[str], timeout: int) -> subprocess.CompletedProcess[str]:
        return web_runtime.run_frontend_command(source_dir, args, timeout)

    def _maybe_build_frontend(self) -> None:
        web_runtime.maybe_build_frontend(
            self.config,
    ...
    def _resolve_frontend_dir(self) -> Path | None:
        return web_runtime.resolve_frontend_dir(self.config, module_path=Path(__file__).resolve())
    ...
    async def run(self) -> None:
        ...
        web_routes.register_web_routes(self, ws_path=ws_path, health_path=health_path)
