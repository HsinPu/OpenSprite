...
from . import coercion as web_settings_coercion
from . import payloads as web_settings_payloads
from . import reload as web_settings_reload
from . import support as web_settings_support
from ....modules.tools.browser_navigation import (
    validate_navigation_url as _validate_navigation_url,
)
from ...browser.factory import (
    browser_cloud_status as _browser_cloud_status,
    cloud_provider_from_config as _cloud_provider_from_config,
)
from ...browser.runtime import AgentBrowserRuntime as _AgentBrowserRuntime
from opensprite.core.logging import logger
from opensprite.integrations.observability.logging import setup_log
from ...search.searxng_http import (
    normalize_searxng_proxy_url as _normalize_searxng_proxy_url,
...
def _browser_command_prefix() -> list[str]:
    return web_runtime.browser_command_prefix()


def _browser_runtime_status() -> dict[str, Any]:
    return web_runtime.browser_runtime_status(_browser_command_prefix())
...
async def _run_browser_doctor_command(
    ...
    *,
    ...
    return await web_runtime.run_browser_doctor_command(
        args,
        timeout=timeout,
...
async def _run_browser_install_command(*, timeout: int = 300) -> dict[str, Any]:
    return await web_runtime.run_browser_install_command(
        timeout=timeout,
        command_prefix=_browser_command_prefix(),
    )
...
async def handle_settings_web_search_searxng_options(adapter: Any, request: web.Request) -> web.Response:
    ...
    try:
        ...
        searxng_proxy = _normalize_searxng_proxy_url(requested_searxng_proxy)
...
async def handle_settings_web_search_update(adapter: Any, request: web.Request) -> web.Response:
    ...
    if "searxng_proxy" in body:
        ...
        try:
            search.searxng_proxy = _normalize_searxng_proxy_url(requested_searxng_proxy)
        except ValueError as exc:
            raise web.HTTPBadRequest(text=str(exc)) from exc
...
async def handle_settings_browser_test(adapter: Any, request: web.Request) -> web.Response:
    ...
    runtime = _AgentBrowserRuntime(
        ...
        cloud_provider=_cloud_provider_from_config(browser),
    )
