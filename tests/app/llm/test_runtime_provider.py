...
from opensprite.app.llm.runtime_provider import (
    DefaultLlmRuntimeFactory,
    ProviderRuntimeError,
    create_llm_from_runtime,
    resolve_provider_runtime,
)
from opensprite.integrations.auth.credentials import add_credential
...
from opensprite.app.llm.provider_factory import create_llm


def test_bootstrap_injects_llm_runtime_factory_for_reload_and_subagents(tmp_path):
    ...
    agent, _message_queue, _cron_manager = asyncio.run(create_agent(config))
    try:
        runtime = agent.reload_llm_from_config(config)

        assert runtime["provider_id"] == "openai"
        assert runtime["model"] == "gpt-test"
        assert runtime["configured"] is True
        assert isinstance(agent._llm_runtime_factory, DefaultLlmRuntimeFactory)
        assert agent.subagents._llm_runtime_factory is agent._llm_runtime_factory
    finally:
...
def test_resolve_copilot_runtime_exchanges_github_token(monkeypatch):
    monkeypatch.setattr(
        "opensprite.app.llm.runtime_auth.get_copilot_api_token",
...
def test_resolve_copilot_runtime_falls_back_to_raw_token_on_exchange_error(monkeypatch):
    from opensprite.integrations.auth.copilot import CopilotAuthError
    ...
    monkeypatch.setattr("opensprite.integrations.auth.copilot.exchange_copilot_token", fail_exchange)
