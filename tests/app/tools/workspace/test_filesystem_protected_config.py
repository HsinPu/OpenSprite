...
from opensprite.app.tools.workspace.filesystem import ApplyPatchTool, EditFileTool, WriteFileTool
...
def test_write_file_blocks_sensitive_user_config_under_home(tmp_path, monkeypatch):
    monkeypatch.setattr("opensprite.modules.tools.workspace_write_policy.Path.home", lambda: tmp_path)
    tool = WriteFileTool(workspace=tmp_path)
