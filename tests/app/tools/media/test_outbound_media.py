...
from opensprite.modules.media.outbound import queue_outbound_media
...
def test_send_media_tool_preserves_structured_queue_errors():
    tool = SendMediaTool(
        queue_media=lambda kind, payload: queue_outbound_media(None, kind, payload),
        get_current_images=lambda: None,
        get_current_audios=lambda: None,
        get_current_videos=lambda: None,
    )
...
def test_outbound_media_queue_reports_invalid_arguments():
    result = queue_outbound_media({}, "sticker", "payload")
    ...
    empty_result = queue_outbound_media({}, "image", " ")
