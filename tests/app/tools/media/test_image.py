...
from opensprite.app.tools.media.image import AnalyzeImageTool, OCRImageTool
...
def test_analyze_image_tool_rejects_non_integer_image_index():
    provider = FakeImageProvider()
    tool = AnalyzeImageTool(MediaRouter(image_provider=provider), get_current_images=lambda: ["img-a"])

    result = asyncio.run(tool.execute(instruction="describe it", image_index="bad"))
