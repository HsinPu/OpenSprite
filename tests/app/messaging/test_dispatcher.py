...
from opensprite.core.contracts.bus_events import RunEvent
...
def test_cron_command_reports_unavailable_while_session_is_resetting():
    class ResettingCronManager:
        async def get_or_create_service(self, session_id: str):
            raise CronSessionResetInProgress(session_id)

    agent = FakeAgent()
    agent.cron_manager = ResettingCronManager()
    queue = MessageQueue(agent)

    result = asyncio.run(queue._handle_cron_command("web:chat-1", "/cron list"))

    assert result == queue.messages.cron.unavailable
...
def test_cron_command_lists_jobs_for_current_session(tmp_path):
    ...
    async def scenario():
        agent = CronAgent()
        agent.cron_manager = _make_cron_manager(tmp_path / "workspace", on_job)
