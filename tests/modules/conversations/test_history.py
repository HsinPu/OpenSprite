...
from opensprite.core.contracts.prompt_history import PreparedPromptHistory
from opensprite.core.contracts.run_events import SEARCH_INDEX_MESSAGE_FAILED_EVENT
...
from opensprite.modules.conversations.history import MessageHistoryService
...
def test_message_history_prepares_prompt_history_without_turn_local_tool_results():
    ...
    for message in [
        ...
        StoredMessage(
            ...
            metadata={
                "llm_reasoning_details": [
                    {"type": "reasoning.text", "text": "prior thinking"}
                ]
            },
        ),
    ...
    prompt_history = asyncio.run(
        service.load_prompt_history("session-1", "current question")
    )

    assert isinstance(prompt_history, PreparedPromptHistory)
...
def test_message_history_saves_visible_user_and_assistant_messages_to_storage_and_index():
    class RecordingHistorySearchStore:
        def __init__(self):
            self.indexed = []
    ...
    async def scenario():
        storage = MemoryStorage()
        history_search_store = RecordingHistorySearchStore()
        service = MessageHistoryService(
            storage=storage,
        ...
        return await storage.get_messages("session-1"), history_search_store.indexed
...
def test_message_history_emits_search_index_failure_event():
    ...
    async def scenario():
        ...
        service = MessageHistoryService(
            storage=storage,
            history_search_store=FailingSearchStore(),
            max_history_getter=lambda: 10,
...
def test_message_history_persists_but_does_not_index_search_history_output():
    class RecordingHistorySearchStore:
        def __init__(self):
            self.indexed = []
    ...
    assert [(message.role, message.tool_name) for message in messages] == [
        ("tool", "search_history")
    ]
    assert indexed == []
