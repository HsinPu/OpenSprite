...
from opensprite.app.messaging.dispatcher import MessageQueue
from opensprite.core.contracts.bus_events import RunEvent, SessionStatusEvent
from opensprite.core.contracts.messages import AssistantMessage
import opensprite.integrations.web.settings.tool_handlers as tool_handlers
from opensprite.config import Config, ProviderConfig
from opensprite.config.defaults import DEFAULT_SEARXNG_URL
from opensprite.integrations.auth.codex import CodexToken, delete_codex_token, save_codex_token
import opensprite.integrations.auth.codex as codex_module
from opensprite.integrations.web import api as web_api
from opensprite.integrations.web.adapter import WebAdapter
from opensprite.integrations.web.routes import register_web_routes
from opensprite.integrations.workspace.paths import get_session_workspace
from opensprite.modules.scheduling.manager import CronManager
...
from opensprite.core.contracts.persistence import StoredBackgroundProcess, StoredMessage
...
from opensprite.integrations.search.searxng_http import SEARXNG_MAX_RESPONSE_BYTES
...
def test_web_adapter_uses_canonical_api_handlers():
    adapter = WebAdapter(mq=None, config={"frontend_auto_build": False})

    assert type(adapter._api) is web_api.WebApiHandlers
...
async def _run_web_browser_settings_manual_test(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr("opensprite.integrations.web.settings.tool_handlers._AgentBrowserRuntime.run", fake_run)
...
def test_browser_diagnostic_classifies_common_failures():
    sandbox = tool_handlers._with_browser_diagnostic(
        {
            "ok": False,
            "stderr": "FATAL: No usable sandbox! Hint: try --args \"--no-sandbox\"",
        }
    )
    missing = tool_handlers._with_browser_diagnostic(
        {
            "ok": False,
            "stderr": "Executable doesn't exist at /tmp/chromium",
        }
    )
    deps = tool_handlers._with_browser_diagnostic(
        {
            "ok": False,
...
async def _run_web_browser_settings_doctor(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(tool_handlers, "_browser_command_prefix", lambda: ["agent-browser"])
    monkeypatch.setattr(tool_handlers, "_run_browser_doctor_command", fake_doctor_command)
...
async def _run_web_browser_settings_install(tmp_path: Path, monkeypatch):
    ...
    async def fake_doctor_command(args, *, timeout=20, launch_args=""):
        ...
        if len(doctor_calls) == 1:
            return tool_handlers._with_browser_diagnostic(
    ...
    monkeypatch.setattr(tool_handlers, "_browser_command_prefix", lambda: ["agent-browser"])
    monkeypatch.setattr(tool_handlers, "_run_browser_doctor_command", fake_doctor_command)
    monkeypatch.setattr(tool_handlers, "_run_browser_install_command", fake_install_command)
...
def test_web_adapter_search_searxng_options(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("opensprite.integrations.web.settings.tool_handlers.httpx.AsyncClient", create_client)
...
async def _run_web_run_events_api():
    ...
    try:
        ...
        async with ClientSession() as session:
            ...
            assert payload["events"][0]["artifact"]["artifact_id"] == "tool:call-1"
            assert payload["events"][0]["artifact"]["status"] == "running"
            assert payload["events"][1]["artifact"]["artifact_id"] == "tool:call-1"
            assert payload["events"][1]["artifact"]["status"] == "completed"
            ...
            assert trace_payload["event_counts"]["total"] == 2
            assert trace_payload["event_counts"]["returned"] == 2
...
async def _run_web_settings_provider_api(tmp_path: Path, monkeypatch):
    ...
    Config.copy_template(config_path)
    applied_log_configs = []
    monkeypatch.setattr("opensprite.integrations.web.settings.tool_handlers.setup_log", lambda log_config: applied_log_configs.append(log_config))
    ...
    class SettingsAgent(EchoAgent):
        ...
        def reload_llm_from_config(self, config):
            ...
            return {"provider_id": config.llm.default, "model": active.model, "configured": is_llm_configured(config)}
    ...
    try:
        ...
        async with ClientSession() as session:
            ...
            assert channel_update_payload["channel"]["registered"] is True
