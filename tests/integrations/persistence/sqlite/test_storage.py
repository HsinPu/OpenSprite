...
from opensprite.core.contracts.run_lifecycle import RUN_STARTED_EVENT
...
def test_sqlite_storage_round_trips_all_message_fields_in_insertion_order(tmp_path):
    storage = SQLiteStorage(tmp_path / "message-round-trip.db")
    messages = [
        StoredMessage(
            role="user",
            content="第一則",
        ...
        StoredMessage(
            ...
            timestamp=20.0,
            tool_name="history_search",
            metadata={"result": [1, 2, 3]},
        ),
    ]

    async def scenario():
        for message in messages:
            await storage.add_message("web:chat-1", message)
        return (
            await storage.get_messages("web:chat-1"),
            await storage.get_messages("web:chat-1", limit=2),
        )

    all_messages, limited_messages = asyncio.run(scenario())
