...
from opensprite.integrations.persistence.sqlite.search import SQLiteSearchStore
...
def test_storage_clear_messages_cascades_shared_search_and_run_state(tmp_path):
    ...
    async def scenario():
        ...
        await _persist_and_index(
            ...
            search,
            "chat-a",
            "shared sqlite cascade marker",
            timestamp=10.0,
        )
        await storage.set_consolidated_index("chat-a", 1)
        await storage.create_run("chat-a", "run-1", created_at=11.0)

        await storage.clear_messages("chat-a")

        return (
            await storage.get_messages("chat-a"),
            await storage.get_consolidated_index("chat-a"),
            await storage.get_run("chat-a", "run-1"),
            await search.search_history("chat-a", "cascade marker"),
        )

    messages, consolidated_index, run, hits = asyncio.run(scenario())
...
def test_search_store_bootstraps_shared_schema_without_storage_adapter(tmp_path):
    db_path = tmp_path / "search-only.db"

    SQLiteSearchStore(db_path)
    ...
    assert {"chats", "messages", "runs", "search_chunks", "search_chunks_fts"} <= table_names
