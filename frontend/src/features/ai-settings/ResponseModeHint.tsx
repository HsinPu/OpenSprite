...
export function ResponseModeHint({ providerId, modelId, mode, enabled = true }: {
  providerId?: ProviderId; modelId?: string; mode: ResponseMode; enabled?: boolean;
}) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    if (!enabled || !providerId || !modelId) return;
  ...
  const text = !providerId || !modelId ? t("models.response.selectModel")
  ...
  return <p className="settings-helper-text response-mode-hint" role="status" aria-live="polite">{text}</p>;
}
