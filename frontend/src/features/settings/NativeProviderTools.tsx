...
import { Alert, Button, Checkbox, Collapse, Form, Modal, Select, Space, Typography } from "antd";
...
import { disabledModelsError, normalizeDisabledModels } from "./providerToolPolicy";
...
export function NativeProviderTools({ provider, name, container, onOverlayChange }: { provider: NativeProviderId; name: string; container?: HTMLElement | null; onOverlayChange?: (open: boolean) => void }) {
  ...
  const save = async () => {
    if (busy || !loaded || disabledModelsError(policy.disabledModels)) return;
  ...
  return <>
    ...
    <Modal title={`${name} · ${t("models.tools.title")}`} getContainer={container ?? false} open={open} onCancel={() => { if (!busy) setOpen(false); }} onOk={() => void save()} confirmLoading={busy} okButtonProps={{ disabled: !loaded || !!disabledModelsError(policy.disabledModels) }} okText={t("common.save")} cancelText={t("common.cancel")}>
      <Space orientation="vertical" size="middle" style={{ width: "100%" }}>
        ...
        <Collapse ghost items={[{ key: "models", label: t("models.custom.advancedTools"), children: <Form.Item label={t("models.tools.disabledModels")} htmlFor={`disabled-${provider}`} layout="vertical" validateStatus={disabledModelsError(policy.disabledModels) ? "error" : undefined} help={disabledModelsError(policy.disabledModels) ? t(disabledModelsError(policy.disabledModels)!) : undefined} extra={t("models.tools.disabledModelsHelp")}><Select id={`disabled-${provider}`} mode="tags" style={{ width: "100%" }} disabled={busy || !loaded} value={policy.disabledModels} onChange={disabledModels => setPolicy({ ...policy, disabledModels: normalizeDisabledModels(disabledModels) })} /></Form.Item> }]} />
      </Space>
    </Modal>
  </>;
}
