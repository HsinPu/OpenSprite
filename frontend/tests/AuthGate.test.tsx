...
afterEach(() => { cleanup(); vi.unstubAllGlobals(); vi.useRealTimers(); window.history.replaceState(null, "", "#new-chat"); });
...
describe("AuthGate", () => {
  ...
  it("does not mount protected application before authentication", async () => {
    const fetchMock = vi.fn().mockImplementation((path: string) => {
      ...
      if (path === "/api/app-info") return Promise.resolve(response({ version: "0.12.0", revision: "development", buildType: "development", dirty: true, installedAt: null }));
