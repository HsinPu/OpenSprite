...
def test_provider_migration_preserves_schema_and_rolls_back(tmp_path):
    ...
    connection.executescript(SCHEMA_SQL.replace(NEW_CHECK, OLD_CHECK).replace("user_version = 19", "user_version = 15"))
...
def test_provider_migration_keeps_existing_conversation_and_run_rows(tmp_path):
    ...
    legacy.executescript(SCHEMA_SQL.replace(NEW_CHECK, OLD_CHECK).replace("user_version = 19", "user_version = 15"))
